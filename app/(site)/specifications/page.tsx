  ...
  return (
    <main className="pt-16 sm:pt-24">
            ...
            <table className="w-full text-sm">
              <tbody>
                {SPEC_ROWS.map((row, i) => (
                  ...
                  >
                    <th className="text-left font-medium text-muted px-1 py-4 align-top w-2/5">
                      {row.label}
                    </th>
                    <td className="text-limestone py-4 align-top">
                      {row.value}
                    </td>
                  </tr>
                ))}
      ...
      <Container className="mt-20 sm:mt-24">
        <h2 className="font-heading text-limestone text-3xl sm:text-4xl">
          Power requirements
        </h2>
        ...
        <div className="card px-6 sm:px-8 py-3 mt-8">
          <table className="w-full text-sm">
            <tbody>
              {POWER.map((row, i) => (
                <tr
                ...
                >
                  ...
                  <td className="text-limestone py-4 align-top">{row.value}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Container className="mt-20 sm:mt-24">
        ...
        <div className="grid sm:grid-cols-2 gap-5 mt-8">
          {SPACE.map((item) => (
            <div key={item.label} className="card p-7">
      ...
      <Container className="mt-20 sm:mt-24">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
          {PHOTOS.gallery.map((photo) => (
            <div key={photo.src} className="card lift overflow-hidden">
              <Image
                src={photo.src}
                alt={photo.alt}
                width={1200}
        ...
        <p className="text-faint text-xs mt-5">
          Photos are of the actual unit. More coming soon — including it set up
          in a {SITE.city} backyard.
        </p>
      </Container>
